...
//		ApplicationContextInitializer，用来初始化指定的 Spring 应用上下文，如注册属性资源、激活 Profiles 等。
		...
		// 设置监听器
//		ApplicationListener这个接口继承了 JDK 的 java.util.EventListener 接口，实现了观察者模式，它一般用来定义感兴趣的事件类型，事件类型限定于 ApplicationEvent的子类，这同样继承了 JDK 的 java.util.EventObject 接口。
...
//		设置系统属性 `java.awt.headless` 的值，默认值为：true，作用：
//		对于一个 Java 服务器来说经常要处理一些图形元素，例如地图的创建或者图形和图表等。
//		这些API基本上总是需要运行一个X-server以便能使用AWT（Abstract Window Toolkit，抽象窗口工具集）。
//		然而运行一个不必要的 X-server 并不是一种好的管理方式。有时你甚至不能运行 X-server,因此最好的方案是运行
//		headless 服务器，来进行简单的图像处理。
...
//		获取ApplicationContextInitializer 的实例名称集合并去重
//		SpringFactoriesLoader.loadFactoryNames作用：
